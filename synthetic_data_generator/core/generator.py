    ...
    def _generate_row(self, person_ids):
        ...
        for col_name, col_spec in self.specifications.items():
            ...
            if col_type == 'date':
                ...
                context_column = col_spec.get('context_column')
                templates = col_spec.get('templates', {})

                if not context_column or not templates:
                    raise ValueError(f"Column '{col_name}' of type 'contextual_text' requires 'context_column' and 'templates' in its specification.")

                if context_column not in row:
                    raise ValueError(f"Context column '{context_column}' must be defined before '{col_name}' in the specification.")

                context_value = row[context_column]

                if context_value in templates:
                    value = random.choice(templates[context_value])
                else:
                    # Fallback if the context value has no template
